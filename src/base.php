...
if (isset($_POST["tagNewPost"]) || isset($_POST["textNewPost"]) || isset($_FILES["fileNewPost"])) {
    if (!empty($_POST["tagNewPost"]) && !empty($_POST["textNewPost"])) {
        if (!str_contains(htmlspecialchars($_POST["tagNewPost"]), "_") && !str_contains(htmlspecialchars($_POST["tagNewPost"]), "&")) {
            ...
            $templateParams["newPostError"] = 'Nel tag hai inserito dei caratteri non ammessi';
        }
    } else {
